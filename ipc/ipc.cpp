...
void osIPC::Server::create(char *pAdd/* = NULL*/)
{
	...
	{
		sprintf_s(m_sAddr, IPC_MAX_ADDR, "%s", pAdd);
	}
	...
	DWORD dwMemBuffSize = sizeof(MemBuff);
	...
	// Clear the buffer
	ZeroMemory(m_pBuf, dwMemBuffSize);
...
void osIPC::Server::retBlock(osIPC::Block* pBlock)
{
	...
	for (;;)
	{
		...
		int nRet = InterlockedCompareExchange(&pBlock->doneRead, 0, 1);
		if (nRet != 1)
		{
...
osIPC::Client::Client(char *connectAddr)
{
	...
	m_hMapFile = OpenFileMapping(	FILE_MAP_ALL_ACCESS,	// read/write access
									...
									(LPCTSTR)m_sMemName);	// name of mapping object
